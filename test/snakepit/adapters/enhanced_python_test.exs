defmodule Snakepit.Adapters.EnhancedPythonTest do
  ...
  describe "command validation" do
    test "validates dynamic commands" do
      ...
      assert :ok ==
               EnhancedPython.validate_command("pipeline", %{
                 "steps" => [%{"target" => "math.sqrt"}]
               })
    ...
    test "validates legacy commands" do
      ...
      assert :ok ==
               EnhancedPython.validate_command("create_program", %{
                 "id" => "test_program",
  ...
  describe "response processing" do
    test "processes dynamic command responses" do
      ...
      assert {:error, "Something went wrong"} =
               EnhancedPython.process_response("call", error_response)
    end
  ...
  describe "key stringification" do
    ...
    test "stringifies keys in nested lists" do
      ...
      assert Enum.at(result, 0)["atom_key"] == "value1"
      assert Enum.at(result, 1)["another_atom"] == "value2"
    end
