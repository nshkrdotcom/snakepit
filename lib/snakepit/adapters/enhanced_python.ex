defmodule Snakepit.Adapters.EnhancedPython do
  ...
  def add_metadata(args, command) do
    Map.merge(args, %{
  ...
  def convert_signature_to_dspy_format(signature) do
    cond do
      is_binary(signature) ->
        signature
      ...
      is_map(signature) ->
        ...
        input_str = if Enum.empty?(input_names), do: "input", else: Enum.join(input_names, ", ")

        output_str =
          if Enum.empty?(output_names), do: "output", else: Enum.join(output_names, ", ")
