...
AUTO_DEMO_DURATION_MS="${SNAKEPIT_AUTO_DEMO_DURATION_MS:-3000}"
...
for script in "${EXAMPLE_SCRIPTS[@]}"; do
  ...
  if [[ "${script}" == "examples/bidirectional_tools_demo.exs" || \
    ...
    if ! run_cmd "${script}" bash -c \
      "cd \"${ROOT_DIR}\" && SNAKEPIT_DEMO_DURATION_MS=${AUTO_DEMO_DURATION_MS} mix run --no-start \"${script}\""; then
      failures+=("${script}")
    fi
    continue
  fi

  if ! run_cmd "${script}" bash -c "cd \"${ROOT_DIR}\" && mix run --no-start \"${script}\""; then
    failures+=("${script}")
  fi
done
...
if [[ "${RUN_SHOWCASE}" -eq 1 ]]; then
  if ! run_cmd "examples/snakepit_showcase (run_all)" bash -c \
    "cd \"${ROOT_DIR}/examples/snakepit_showcase\" && mix deps.get && \
     mix run --eval 'Snakepit.run_as_script(fn -> SnakepitShowcase.DemoRunner.run_all() end)'"; then
...
if [[ "${RUN_LOADTEST}" -eq 1 ]]; then
  ...
  if ! run_cmd "examples/snakepit_loadtest (basic)" bash -c \
    "cd \"${ROOT_DIR}/examples/snakepit_loadtest\" && mix deps.get && \
     mix run --eval 'Snakepit.run_as_script(fn -> SnakepitLoadtest.Demos.BasicLoadDemo.run(${LOADTEST_BASIC_WORKERS}) end)'"; then
  ...
  if ! run_cmd "examples/snakepit_loadtest (stress)" bash -c \
    "cd \"${ROOT_DIR}/examples/snakepit_loadtest\" && \
     mix run --eval 'Snakepit.run_as_script(fn -> SnakepitLoadtest.Demos.StressTestDemo.run(${LOADTEST_STRESS_WORKERS}) end)'"; then
    failures+=("examples/snakepit_loadtest stress")
  fi

  if ! run_cmd "examples/snakepit_loadtest (burst)" bash -c \
    "cd \"${ROOT_DIR}/examples/snakepit_loadtest\" && \
     mix run --eval 'Snakepit.run_as_script(fn -> SnakepitLoadtest.Demos.BurstLoadDemo.run(${LOADTEST_BURST_WORKERS}) end)'"; then
  ...
  if ! run_cmd "examples/snakepit_loadtest (sustained)" bash -c \
    "cd \"${ROOT_DIR}/examples/snakepit_loadtest\" && \
     ...
     mix run --eval 'Snakepit.run_as_script(fn -> SnakepitLoadtest.Demos.SustainedLoadDemo.run(${LOADTEST_SUSTAINED_WORKERS}) end)'"; then
