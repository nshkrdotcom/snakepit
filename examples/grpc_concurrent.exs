...
defmodule ConcurrentExample do
  def run(pool_size) do
    ...
    worker_counts =
      Enum.reduce(results, %{}, fn {_id, _wait, _duration, worker_id}, acc ->
    ...
    # Create multiple sessions and use them concurrently
    sessions = for i <- 1..3, do: "concurrent_session_#{i}"

    # Initialize sessions concurrently with ping + echo (available tools)
    init_tasks =
      for session <- sessions do
        Task.async(fn ->
          ...
          {:ok, _} =
            Snakepit.execute_in_session(session, "echo", %{
              message: "Session #{session} initialized"
            })
    ...
    initialized_sessions = Task.await_many(init_tasks, 5_000)
    IO.puts("  Initialized #{length(initialized_sessions)} sessions concurrently")
    ...
    update_tasks =
      for i <- 1..5 do
        for session <- sessions do
          Task.async(fn ->
            {:ok, result} =
              Snakepit.execute_in_session(session, "add", %{
                a: i,
                b: :rand.uniform(10)
              })
    ...
    affinity_tasks =
      for _ <- 1..3 do
        Task.async(fn ->
          {:ok, result1} = Snakepit.execute_in_session("concurrent_session_1", "ping", %{})
          {:ok, result2} = Snakepit.execute_in_session("concurrent_session_1", "ping", %{})
          {result1["worker_id"], result2["worker_id"]}
        end)
      end
