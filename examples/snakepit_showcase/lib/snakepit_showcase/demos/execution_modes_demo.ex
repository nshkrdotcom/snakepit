defmodule SnakepitShowcase.Demos.ExecutionModesDemo do
  ...
  defp demonstrate_streaming_execution do
    ...
    callback_fn = fn chunk ->
      step = Map.get(chunk, "step") || Map.get(chunk, :step)
      total = Map.get(chunk, "total") || Map.get(chunk, :total)
      progress = Map.get(chunk, "progress") || Map.get(chunk, :progress)
      message = Map.get(chunk, "message") || Map.get(chunk, :message) || inspect(chunk)

      IO.puts("     Step #{step}/#{total}: #{progress}% - #{message}")

      if Map.get(chunk, "is_final") || Map.get(chunk, :is_final) do
  ...
  defp demonstrate_performance_comparison do
    ...
    IO.puts(
      "   Stateless: #{stateless_time}ms (#{round(iterations / (stateless_time / 1000))} ops/sec)"
    )

    IO.puts(
      "   Session-based: #{session_time}ms (#{round(iterations / (session_time / 1000))} ops/sec)"
    )
