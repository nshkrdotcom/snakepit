defmodule SnakepitShowcase.Demos.SessionDemo do
  ...
  defp demo_session_creation do
    ...
    {:ok, result} =
      Snakepit.execute_in_session(session_id, "init_session", %{
        user_id: 123,
        config: %{
          "model" => "gpt-4",
          "temperature" => 0.7
  ...
  defp demo_session_affinity(session_id) do
    ...
    # Execute multiple commands to observe affinity in hint mode
    IO.puts("   Executing 10 commands to observe worker affinity (hint mode)...")

    worker_pids =
      Enum.map(1..10, fn i ->
        {:ok, result} =
          Snakepit.execute_in_session(session_id, "get_worker_info", %{
            call_number: i
          })
    ...
    if length(unique_pids) == 1 do
      IO.puts("   ✅ All commands executed on same worker (hint mode): #{hd(unique_pids)}")
    else
      IO.puts("   ⚠️ Commands executed on different workers: #{inspect(unique_pids)}")
